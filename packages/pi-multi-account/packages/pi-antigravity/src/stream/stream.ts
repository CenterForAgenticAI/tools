...
export function buildRequest(
  ...
  const projectedContext = projectContext(context);
  const injectedSkills = projectedContext.messages.flatMap((msg) =>
    msg.role === "user" ? skillBlocks(msg.content) : [],
  );
  const systemParts = projectedContext.systemPrompt
    ? [{ text: sanitizeText(projectedContext.systemPrompt) }]
  ...
  const contents = convertMessages(model, projectedContext, runtimeModel);
  const hasUserText = contents.some(
    (turn) =>
      turn.role === GeminiRole.User &&
      turn.parts.some((part) => "text" in part && Boolean(part.text.trim())),
  );
  if (!hasUserText && (injectedSkills.length > 0 || Boolean(projectedContext.systemPrompt))) {
    contents.unshift({
  ...
  const tools = convertTools(projectedContext.tools, isClaude || model.id.startsWith("gpt-oss-"));
  if (tools) {
    request.tools = tools;
  }
  ...
  void runWithDiagnostics(async () => {
    ...
    try {
      ...
      for (let emptyAttempt = 0; emptyAttempt <= 2; emptyAttempt++) {
        let primaryRateLimitFailure:
          { status: 429; text: string; endpoint: string; runtimeModel: string } | undefined;
        ...
        for (let candIdx = 0; candIdx < runtimeCandidates.length; candIdx++) {
          ...
          for (const endpoint of endpointCandidates()) {
            ...
            if (response.status === 429 && !primaryRateLimitFailure) {
              primaryRateLimitFailure = { status: 429, text: lastText, endpoint, runtimeModel };
            }
        ...
        if (!response || !response.ok) {
          if (antigravityEnv("DEBUG_DUMP") === "1") {
            try {
              ...
              await writeFile(
                join(tmpdir(), "antigravity-last-request.json"),
                JSON.stringify(
                  {
          ...
          const selectedRateLimitFailure =
            response?.status === 404 ? primaryRateLimitFailure : undefined;
          const failureStatus = selectedRateLimitFailure?.status ?? response?.status;
          const failureText = selectedRateLimitFailure?.text ?? lastText;
          const failureRuntimeModel = selectedRateLimitFailure?.runtimeModel ?? runtimeModel;
          if (selectedRateLimitFailure) {
            setLastStatus(selectedRateLimitFailure.status);
            setLastEndpoint(selectedRateLimitFailure.endpoint);
            setLastResolvedRuntimeModel(selectedRateLimitFailure.runtimeModel);
          }
          const friendly = friendlyAntigravityError(failureStatus, failureText);
          if (failureStatus === 429 && /Quota reached\./i.test(friendly)) {
            throw new Error(friendly);
          }
          throw new Error(
            `Antigravity API error (${failureStatus ?? "no response"}, ${formatRequestDiagnostics({ projectId, runtimeModel: failureRuntimeModel })}): ${friendly}`,
          );
        }
