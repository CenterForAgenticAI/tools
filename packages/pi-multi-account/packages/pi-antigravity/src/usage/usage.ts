...
async function postJson(
  ...
  for (const endpoint of endpointCandidates()) {
    try {
      ...
      // An abort must stop this endpoint-candidate loop immediately instead of being
      // treated as "this endpoint failed, try the next one." Check the signal that was
      // passed into *this* call, not the shape of the caught error, so it works
      // uniformly regardless of how a given transport raises the abort.
...
async function loadCodeAssistSafe(token: string, signal?: AbortSignal) {
  try {
    ...
    // An abort must not be folded into "this leg just came back empty": that would let
    // fetchAccountUsage resolve with a partial, silently-truncated result instead of
    // rejecting.
    if (signal?.aborted) throw error;
    return null;
  }
...
> {
  try {
    ...
    // An abort must not be folded into "this leg just came back empty": see
    // loadCodeAssistSafe above for the same reasoning.
    if (signal?.aborted) throw error;
...
export async function fetchAccountUsage(
  ...
  // to minimize command execution latency. Drain every settlement (not just the
  // first rejection): Promise.all's first-reject alone would not guarantee the
  // other two legs' own transport calls have actually stopped, which matters once
  // an abort is in flight.
  const [assistSettled, summarySettled, availableSettled] = await Promise.allSettled([
    loadCodeAssistSafe(creds.token, options?.signal),
    fetchQuotaSummarySafe(creds.token, options?.signal),
    fetchAvailableModelsCatalog(creds.token, initialProjectId, options?.signal),
  ]);

  // An abort takes priority over a leg that merely came back empty: any genuine
  // rejection here (an abort, or the catalog leg's own "no endpoint available"
  // once every candidate failed) must reject the exported promise rather than be
  // masked by a sibling leg that happened to settle first.
  if (assistSettled.status === "rejected") throw assistSettled.reason as unknown;
  if (summarySettled.status === "rejected") throw summarySettled.reason as unknown;
  if (availableSettled.status === "rejected") throw availableSettled.reason as unknown;

  const assistResult = assistSettled.value;
  const summaryRes = summarySettled.value;
  const available = availableSettled.value;
